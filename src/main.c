...
Window* window;
TextLayer *text_layer, *battery_text_layer, *date_text_layer, *update_at_a_glance, *day_week_layer;
...
char dayWeekBuffer[] = "Воскресенье"; //it will be enougth for EN and RU langs
...
void glance_this(const char *updateText, bool vibrate, int vibrateNum, int animationLength) {
	...
	if (booted == true) {
		...
		GRect start01 = GRect(0, 200, 140, 168);
		GRect finish01 = GRect(0, 145, 140, 168);
		GRect start02 = GRect(0, 145, 140, 168);
		GRect finish02 = GRect(0, 200, 140, 168);
...
void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
	...
	text_layer_set_text(day_week_layer, dayWeekBuffer);
...
#ifndef RU
    #define bt_off "Bluetooth disconnected"
    #define bt_on "Bluetooth connected"
#else
    #define bt_off "Bluetooth отключен"
    #define bt_on "Bluetooth подключен"
#endif
	if (connected == 0) {
		bt_connected = gbitmap_create_with_resource(RESOURCE_ID_bt_disconnected);
		glance_this(bt_off, 1, 3, 5000);
	}
	if (connected == 1) {
		bt_connected = gbitmap_create_with_resource(RESOURCE_ID_bt_connected);
		glance_this(bt_on, 1, 2, 3000);
	}
...
void window_load(Window *window) {
	...
	//Bluetooth layer
	bt_connected = gbitmap_create_with_resource(RESOURCE_ID_bt_disconnected);
	bt_connected_layer = bitmap_layer_create(GRect(0, -5, 40, 40)); //old value of Y is 135
	...
	update_at_a_glance = textLayerInit(GRect(0, 200, 140, 168), GColorBlack, GColorClear, FONT_KEY_GOTHIC_18, GTextAlignmentCenter);
	layer_add_child(window_layer, (Layer*) update_at_a_glance);

	//Day of week text layer
	day_week_layer = textLayerInit(GRect(0, 115, 144, 140), GColorBlack, GColorClear, FONT_KEY_ROBOTO_CONDENSED_21, GTextAlignmentCenter);
	layer_add_child(window_layer, text_layer_get_layer(day_week_layer));
	...
	//Battery text layer
	battery_text_layer = textLayerInit(GRect(110, 5, 40, 40), GColorBlack, GColorClear, FONT_KEY_ROBOTO_CONDENSED_21, GTextAlignmentCenter); //old value of Y is 145	
...
void window_unload(Window *window) {
	...
	text_layer_destroy(date_text_layer);
    	text_layer_destroy(day_week_layer);
...
void init() {
		...
		num_animation = 1;
	{
		GRect start_text,
				...
				finish_batt = GRect(110, 5, 40, 40),
				finish_bt = GRect(0, -5, 40, 40),
                finish_week = GRect(0, 115, 140, 140);
		switch (num_animation) {
			case 1:
			{
				...
				start_bt = GRect(115, 600, 40, 40);
                		start_week = GRect(25, 600, 140, 168);
				break;
			}
			case 2:
			{
				...
				start_batt = GRect(0, -600, 40, 40);
				start_bt = GRect(115, -600, 40, 40);
                		start_week = GRect(0, -600, 140, 168);
				break;
			}
				//Mixed
			case 3:
			{
				...
				start_bt = GRect(-200, 0, 40, 40);
                		start_week = GRect(-200, 0, 140, 168);
				break;
			}
			case 4:
			{
				...
				start_bt = GRect(300, 0, 40, 40);
                		start_week = GRect(300, 0, 140, 168);
				break;
			}
			case 5:
			{
				...
				start_bt = GRect(-300, -250, 40, 40);
                		start_week = GRect(-300, 250, 140, 168);
				break;
			}
		}
		...
		animate_layer(text_layer_get_layer(date_text_layer), &start_date, &finish_date, 1130, 10);
        	animate_layer(text_layer_get_layer(day_week_layer), &start_week, &finish_week, 1100, 10);
